...
*/
if(getuid()){
  	printf("You should be root to run yaph with nmap engine\n");
	exit(EXIT_SUCCESS);
}
//fstat
}
	...
	while(fgets(buff,sizeof(buff),cnf))
	{
            if(buff[strspn(buff," ")]!='#')
            {
                if(strstr(buff,"MaxCheckThreads")){
                    char *c=strchr(buff,'=');
                    int x=atoi(c?c+1:"-1");
                    if(x>0)globals->paral_checks=x;
                }else if(strstr(buff,"TcpReadTimeOut")){
                    char *c=strchr(buff,'=');
                    int x=atoi(c?c+1:"-1");
                    if(x>0)globals->tcp_read_time_out=x;
                }else if(strstr(buff,"TcpConnectTimeOut")){
                    char *c=strchr(buff,'=');
                    int x=atoi(c?c+1:"-1");
                    if(x>0)globals->tcp_connect_time_out=x;
...
if(argc<2)usage();
opterr=0;
while(-1!=(c=getopt_long_only(argc,argv,"p:s:P:o::h",long_options,&option_index)))
	{
		switch(c)
		{
			case 0:
